...
public class CoffeeStatisticServiceTest {
    ...
    public void testFavoriteSpot() throws Exception {
        Map<Date, Location> values = new HashMap<>();
        when(location1.getLatitude()).thenReturn(0.1111111, 0.11);
        when(location1.getLongitude()).thenReturn(0.1111111, 0.11);

        when(location2.getLatitude()).thenReturn(0.1122222, 0.11);
        when(location2.getLongitude()).thenReturn(0.1122222, 0.11);

        when(location3.getLatitude()).thenReturn(0.22222, 0.22);
        when(location3.getLongitude()).thenReturn(0.22222, 0.22);
        ...
        verify(location1).setLongitude(0.11);
        verify(location1).setLatitude(0.11);
        verify(location2).setLongitude(0.11);
        verify(location2).setLatitude(0.11);
        verify(location3).setLongitude(0.22);
        verify(location3).setLatitude(0.22);

        assertEquals(0.11, favorite.getLongitude(), 0.0000001);
        assertEquals(0.11, favorite.getLatitude(), 0.0000001);
    }
}